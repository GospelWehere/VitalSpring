...
CREATE TABLE appointment (
  ...
  practitioner_id BIGINT NOT NULL REFERENCES practitioner(practitioner_id) ON DELETE RESTRICT,
  slot_id BIGINT NOT NULL REFERENCES availability_slot(slot_id) ON DELETE RESTRICT,
...
-- Only one *active* appointment may occupy a slot. Cancelled and no-show
-- appointments release the slot for rebooking (the cancel flow reopens it),
-- while this partial unique index preserves the double-booking barrier for
-- concurrent active confirmations (AT-01).
CREATE UNIQUE INDEX uq_appointment_active_slot ON appointment (slot_id)
